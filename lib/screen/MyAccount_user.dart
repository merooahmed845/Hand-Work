...
class MyAccountUser extends StatefulWidget {
  @override
  _MyAccountUserState createState() => _MyAccountUserState();
}
...
class _MyAccountUserState extends State<MyAccountUser> {
  ...
  void _onItemTapped(int index) {
    setState(() {
      ...
      if (_selectedIndex == 0) {
        ...
        // نحن بالفعل في صفحة الحساب، لذا لا نفعل شيئًا.
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(builder: (context) => SearchPage()), // العودة إلى صفحة البحث
        );
        return false;
      },
      child: Scaffold(
